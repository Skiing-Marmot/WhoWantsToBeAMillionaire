...
public class Game {
	...
	public void loadSavedState() {
		...
		SharedPreferences sharedGamePreferences = activity
				.getSharedPreferences(SHARED_PREF_GAME_FILE_NAME,
						Activity.MODE_PRIVATE);
		questionNumber = sharedGamePreferences.getInt(
				SHARED_PREF_QUESTION_NUMBER, 1);
		...
		isUsedPhoneJoker = sharedGamePreferences.getBoolean(SHARED_PREF_PHONE,
				false);
		SharedPreferences sharedSettingsPreferences = activity
				.getSharedPreferences(SHARED_PREF_SETTINGS_FILE_NAME,
						Activity.MODE_PRIVATE);
		playerName = sharedSettingsPreferences.getString(SHARED_PREF_NAME_KEY,
				"unknown");
	}
	...
	public boolean canUseJoker() {
		SharedPreferences sharedSettingsPreferences = activity
				.getSharedPreferences(SHARED_PREF_SETTINGS_FILE_NAME,
						Activity.MODE_PRIVATE);
		int allowedJokers = sharedSettingsPreferences.getInt(
				SHARED_PREF_JOKERS_NB, 0);
		int usedJokersNb = 0;
	...
	public void saveScore(int scoreToSave) {
		...
		db.open();
		db.insertResult(new HighScore(playerName,
				scoreToSave));
		db.close();
		// On the server
	...
	private void sendScore(final String playerName, final int score) {
		...
		activity.new SendPlayerScore().execute(playerName,
